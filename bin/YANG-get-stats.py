...
if __name__ == "__main__":
    ...
    parser.add_argument("--draftpathdiff", default= ietf_directpry + "/draft-with-YANG-diff/",
...
for prefix in ["IEEEYANGPageCompilation_", "IETFYANGPageCompilationCiscoAuthors_", "IETFYANGPageCompilation_", "LithiumODLPageCompilation_"]:
    ...
    if debug_level > 0:
        print file_name_containing_keyword(all_files, prefix, debug_level)[-1]   
    bash_command = "grep" + " " + ".txt" + " " + args.htmlpath + file_name_containing_keyword(all_files, prefix, debug_level)[-1]  + " | wc -l " +" 2>&1"
    total_result = os.popen(bash_command).read()
    total_result = total_result.rstrip("\r\n")
    if prefix == "LithiumODLPageCompilation_":
        total_result = str(int(failed_result) + int(passed_with_warning_result) + int(passed_result))
...
if debug_level > 0:
    print "DEBUG: " + " print the diff between files and files_no_strict lists, so the files with xym extraction issues: " + files_diff
