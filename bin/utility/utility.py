...
def _resolve_maturity_level(ietf: t.Optional[str], document_name: str):
    if ietf == 'ietf-rfc':
        return 'ratified'
...
def _resolve_working_group(name_revision: str, ietf: str, document_name: str):
    if ietf == 'ietf-rfc':
...
def _generate_ths(versions: dict, ietf: t.Optional[dict]) -> t.List[str]:
    ths = list()
    option = '--lint'
    if ietf is not None:
        option = '--ietf'
...
def _generate_compilation_result_file(module_data: dict, compilation_results: dict, result_html_dir: str,
                                      is_rfc: bool, versions: dict, ietf: t.Optional[dict]) -> str:
    ...
    context = {'result': compilation_results,
                'ths': _generate_ths(versions, ietf)}
...
def check_yangcatalog_data(config: configparser.ConfigParser, yang_file_pseudo_path: str, new_module_data: dict,
    ...
    if name_revision in all_modules:
        ...
        if new_module_data.get('compilation-status') \
            ...
            if is_rfc and ietf_metadata is None:
                pass
            else:
                update = True
        ...
        if new_module_data.get('compilation-status') is not None:
            file_url = _generate_compilation_result_file(module_data, compilation_results,
                                                         result_html_dir, is_rfc, versions, ietf_metadata)
        ...
        if ietf_metadata is not None and module_data.get('organization') == 'ietf':
            wg = _resolve_working_group(name_revision, ietf_metadata, new_module_data['reference'])
            if (module_data.get('ietf') is None or module_data['ietf']['ietf-wg'] != wg) and wg is not None:
                update = True
                module_data['ietf'] = {}
                module_data['ietf']['ietf-wg'] = wg

        mat_level = _resolve_maturity_level(ietf_metadata, new_module_data['reference'])
